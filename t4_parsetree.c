	...
	)
{
	...
	s = word;
	switch(p->type) {
		case type_constant: sprintf (word, "c(%d)", p->con.value); break;
		case type_variable: sprintf (word, "var(%s)", p->var.str); break;	//war type_variable sprintf (word, "id(%c)", p->id.i + 'A');
		case type_operator:
			switch(p->opr.oper){
				case WHILE: s = "schleife"; break;
				case IF: s = "wenn"; break;
				case PRINT: s = "hau_raus"; break;
				//case '\t':s="[]";break;
				case '\n': s = "|";
					if(x=2)
						z = 1;
	...
	*cm = c + w / 2;
	...
	/* node is leaf */
	if (p->type == type_constant || p->type == type_variable || p->opr.nops == 0) {
